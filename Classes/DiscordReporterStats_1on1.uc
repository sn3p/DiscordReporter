...
function OnGameDetails()
{
  ...
  //   SendMessage(">>"$conf.colHead$" Current Score:"$ircColor$"4 "$sName_1$" - Login: "$Spec.ServerMutate("getlogin "$sName_1)$" "$sScore_1$ircColor$":"$ircColor$"12"$ircBold$ircBold$sScore_2$" "$sName_2$" - Login: "$Spec.ServerMutate("getlogin "$sName_2));
  // else
